...
- Use inline code to make file paths clickable in VS Code.
...
/Users/alex/repo/api/user.ts:87
...
- When searching for text or files, prefer using `rg` or `rg --files` respectively because `rg` is much faster than alternatives like `grep` or `ls -R`. (If the `rg` command is not found, then use alternatives.)
